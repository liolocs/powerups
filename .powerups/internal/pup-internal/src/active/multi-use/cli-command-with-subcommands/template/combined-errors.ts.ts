...
export default function(variables: Record<string, string>): string {
  const { commandName, errorCases, subcommandErrorCases } = variables;
  const varName = toCamelCase(commandName);
  const pascalName = toPascalCase(commandName);

  const parentErrors: Array<{ name: string; text: string }> =
    JSON.parse(errorCases || "[]");
  const subErrors: Array<{ name: string; text: string }> =
  ...
  if (allErrors.length === 0) {
    ...
    errorCasesCode = allErrors.map(e => {
