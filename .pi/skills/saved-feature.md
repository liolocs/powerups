---
name: powers-feature
description: "Search and run powers features for new additions"
---
Search and run powers features for new additions.
...
pattern — consider using `powers-template` instead." Only proceed with
...
rather than generating something new — use the `powers-capture` skill
instead of this one.

1. Run `powers feature search -q="<what the user wants to build>"`.
2. If a feature matches (score > 0):
   - Run `powers feature apply <name> --<variable-name>=<value> ... -d` to preview.
...
3. If no feature matches:
   ...
   - If yes, run `powers feature create -n=<short-name> -i="<intent keywords>" -v="..." -ov="..." -o='...'`,
     ...
     and run `powers feature validate -n=<name>`.
