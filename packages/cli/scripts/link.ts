#!/usr/bin/env bun
/**
 * Dev linker for @liolocs/powerups-cli.
 *
 ...
 * (`pnpm --filter @liolocs/powerups-cli build`); the global symlink already points here.
 */
...
async function main(): Promise<void> {
  ...
  if (await binRef.exists()) {
    log("✓ lib/bin.js already built (rebuild with `pnpm --filter @liolocs/powerups-cli build`)\n");
  } else {
  ...
  log(`• linking ${pkg.name ?? "@liolocs/powerups-cli"} globally ...\n`);
