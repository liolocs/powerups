...
test.case("ts runner renders a template with a default function export", async (assert) => {
    await fs.create(tmpDir);
    const templatePath = tmpDir.append("/button.ts");
    await templatePath.write("export default function({ componentName, theme }: Record<string, string>) {\n" +
...
test.case("ts runner throws template_not_found for missing file", async (assert) => {
    await fs.create(tmpDir);
...
test.case("ts runner throws invalid_ts_template when no default function", async (assert) => {
    await fs.create(tmpDir);
