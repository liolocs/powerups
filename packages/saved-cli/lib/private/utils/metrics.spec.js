...
test.case("logRun creates metrics file and appends entries", async (assert) => {
    await reset();
    await logRun({ pattern: "ui-component", characters: 500 });
...
test.case("readMetrics returns parsed entries", async (assert) => {
    await reset();
    await logRun({ pattern: "ui-component", characters: 500 });
    await logRun({ pattern: "ui-component", characters: 300 });
    await logRun({ pattern: "api-route", characters: 1200 });
...
test.case("logRun appends to existing file without overwriting", async (assert) => {
    await reset();
    await logRun({ pattern: "first", characters: 10 });
    const afterFirst = await metricsPath.text();
    await logRun({ pattern: "second", characters: 20 });
