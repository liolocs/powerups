 ...
 * "-d" -> "d"
 */
export function normalizeFlagName(flag) {
    // Strip leading -- or -
 ...
 */
export function extractVariables(rawFlags, declaredVariables, excludeFlags) {
    ...
    for (const declared of declaredVariables) {
        const matched = Object.keys(result).find(k => k.toLowerCase() === declared.toLowerCase());
        if (!matched) {
            throw patternRunErrors.missing_variable(declared, toKebabCase(declared));
        }
    }
    return result;
}
