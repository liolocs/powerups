...
description: "Brainstorm a plan using powers powers"
---
Brainstorm a plan using powers powers.

**Announce at start:** "I'm using the powers-brainstorm skill to plan this work."

1. Explore the project context:
   - Check recent commits, existing files, and docs.
   - Run `pwrs search -q="<what the user wants to plan>"` to see if
     any powers already relate to this area.
   - If the search returns "No matching powers found", check whether
     `.powers/active/multi-use/` or
     `.powers/active/single-use/` exist and
...
2. Run `pwrs search -q="<intent>"` for every identified intent
3. Run `pwrs info <name>` for every match found
4. Recorded the results (matches found or "no matching") for each search

If you skip the search step, you risk duplicating existing work and planning
the wrong approach. Search is not optional.
</HARD-GATE>

3. Identify the key intents (topics, domains, capabilities) this plan touches.
   For each intent, run `pwrs search -q="<intent>"` to find existing
   powers that may be reusable. For every match, run
   `pwrs info <name>` to understand what variables
   and files are available. Record all results.

4. From the search results, determine:
   - Which existing powers can be reused for this plan (list by
...
5. For work that doesn't fit an existing item, classify each piece as:
   ...
   - It only adds npm dependencies (use `packageDependencies` in the
     power's `instructions.json` instead — the CLI handles `package.json`
   ...
   powers before the concrete implementation exists and is
   ...
   concrete, standalone powers. If two planned powers clearly share
   a file structure, note it as a future subtemplate extraction candidate
   — to be extracted after both powers are built and verified working.
   See the main instruction file for subtemplate mechanics.

6. Present the plan in sections, covering:
   - Overview of the approach
   - Existing powers to reuse (with names and key variables)
...
8. Self-review the plan:
   ...
   - Is the build-first principle followed? (No "create power first" steps.)
...
11. Once the user approves, transition to implementation by invoking the
    `powers-implement` skill to execute the plan.
