...
export default function(variables: Record<string, string>): string {
  const { commandName, subcommandName, subcommandDescription, subcommandErrorCases } = variables;
  const parentVar = toCamelCase(commandName);
  const subVar = parentVar + toPascalCase(subcommandName);
  const parentPascal = toPascalCase(commandName);
...
import { ${parentPascal}ErrorCode } from "#errors/${commandName}Errors";
...
test.case("${subcommandName} has correct name and description", async assert => {
  ...
  assert(${subVar}.description).equals("${subcommandDescription}");
});
${errorTests}`;
}