...
test.case("CLI program returns help message with no args", async (assert) => {
    ...
    const allOutput = rcli.print.calls
        .map(call => call[0])
        .join("");
...
test.case("CLI program delegates --help to a command's own help", async (assert) => {
    const createFlag = {
        name: "name",
        long: "name",
        short: "n",
...
test.case("CLI program runs an unknown command and shows help", async (assert) => {
    const createCommand = new Command({
        name: "create",
        description: "Create a new project",
        flags: [],
...
test.case("CLI program groups subcommands under their own section", async (assert) => {
