...
export default async function tsRunner({ templatePath, variables }) {
 ...
 */
async function directImport(templatePath, variables) {
    try {
 ...
 */
async function childProcessImport(templatePath, variables) {
    const os = await import("node:os");
    const { execFile } = await import("node:child_process");
    const { promisify } = await import("node:util");
    const execFileAsync = promisify(execFile);
    // Write temp runner .mjs
    const tmpFile = fs.ref(`${os.tmpdir()}/dryai-runner-${Date.now()}.mjs`);
    const runnerContent = [
    ...
    try {
        const { stdout } = await execFileAsync(runtime.bin, ["--experimental-strip-types", tmpFile.path], {
            env: {
                ...process.env,
        ...
        const stderr = error_.stderr ?? "";
        const message = error_ instanceof Error
            ? `${error_.message}${stderr ? `: ${stderr}` : ""}`
            : String(error_);
        throw patternRunErrors.template_execution_error(templatePath.name, message);
    }
    finally {
