...
/**
 * Copy the package content tree (multi-use + single-use folders and every
...
export async function copyActiveStructure({
  ...
  await fs.create(destDir);

  for (const typeFolder of [MULTI_USE_FOLDER, SINGLE_USE_FOLDER]) {
    const srcTypeDir = srcDir.append(`/${typeFolder}`);
    if (await fs.exists(srcTypeDir)) {
      const destTypeDir = destDir.append(`/${typeFolder}`);
 ...
 * under the destination package content tree, skipping any that already exist.
 */
export async function copySubPowerUps({
  ...
  for (const [subName, info] of collected) {
    const typeFolder = info.type === "multi-use"
      ? MULTI_USE_FOLDER
      : SINGLE_USE_FOLDER;
    const destPowerDir = destDir.append(`/${typeFolder}/${subName}`);
