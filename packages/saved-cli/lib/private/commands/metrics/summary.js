...
const summary = new Command({
    ...
    action: async (props) => {
        ...
        const colWidths = headers.map((h, i) => Math.max(h.length, ...dataRows.map(r => r[i].length), totalRow[i].length));
        // Row formatter: pattern left-aligned, numbers right-aligned
        const formatRow = (pattern, runs, chars, tokens) => `${pattern.padEnd(colWidths[0])}   ${runs.padStart(colWidths[1])}   ${chars.padStart(colWidths[2])}   ${tokens.padStart(colWidths[3])}`;
        // Print table
