...
test.case("extractVariables returns camelCase record", async (assert) => {
    const result = extractVariables([{ flag: "--component-name", value: "Button" }, { flag: "--theme", value: "dark" }], ["ComponentName", "theme"], ["--dry-run", "-d", "--help", "-h"]);
...
test.case("extractVariables excludes declared flags", async (assert) => {
    ...
    const result = extractVariables([{ flag: "--dry-run", value: "true" }, { flag: "--component-name", value: "Button" }], ["ComponentName"], ["--dry-run", "-d", "--help", "-h"]);
    assert(result.componentName).equals("Button");
...
test.case("extractVariables ignores undeclared extra flags", async (assert) => {
    const result = extractVariables([{ flag: "--component-name", value: "Button" }, { flag: "--extra", value: "ignored" }], ["ComponentName"], ["--dry-run", "-d", "--help", "-h"]);
    assert(result.componentName).equals("Button");
...
test.case("extractVariables throws on missing declared variable", async (assert) => {
    let threw = false;
    try {
        extractVariables([{ flag: "--theme", value: "dark" }], ["ComponentName", "theme"], ["--dry-run", "-d", "--help", "-h"]);
    }
    catch (e) {
        threw = true;
        assert(e instanceof CodeError).true();
        assert(e.code).equals("missing_variable");
    }
    assert(threw).true();
});
test.case("extractVariables matches case-insensitively", async (assert) => {
    // Declared as PascalCase, flag as kebab -> camelCase
    const result = extractVariables([{ flag: "--component-name", value: "Button" }], ["ComponentName"], []);
    assert(result.componentName).equals("Button");
});
