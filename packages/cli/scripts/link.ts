 ...
 * (`pnpm --filter @powerups/cli build`); the global symlink already points here.
 */
...
async function main(): Promise<void> {
  ...
  if (await binRef.exists()) {
    log("✓ lib/bin.js already built (rebuild with `pnpm --filter @powerups/cli build`)\n");
  } else {
  ...
  log(`• linking ${pkg.name ?? "@powerups/cli"} globally ...\n`);
